...
static void Inverse_S_by_Cholesky(int Mc_AN, double ****OLP0, double ***S12);
static void TSB_Lanczos(int Mc_AN, int spin, double *****Hks);
...
double RecursionS_G(int SCF_iter,
                    double *****Hks,
                    double ****OLP0,
                    double *****CDM,
                    double *****EDM,
  ...
  ****************************************************/
  ...
  if (SCF_iter==1){
    for (Mc_AN=1; Mc_AN<=Matomnum; Mc_AN++){
      Inverse_S_by_Cholesky(Mc_AN, OLP0, S12); 
    }
  }
...
void TSB_Lanczos(int Mc_AN, int spin, double *****Hks)
{
  static int firsttime=1;
            ...
            INTEGER *ipiv, dcomplex *a1d, dcomplex *work)
{
  ...
  ****************************************************/
    ...
    ****************************************************/
    ...
    if ( 0<(RNUM[ct_AN]-itnum+1) ) 
      rl_min = RNUM[ct_AN] - itnum + 1; 
    else 
      rl_min = 0;

    for (rl=RNUM[ct_AN]; rl_min<=rl; rl--){
...
void Inverse_S_by_Cholesky(int Mc_AN, double ****OLP0, double ***S12)
{
  char  *UPLO="U";
  ...
  for (j=0; j<N; j++) {
    m = j*N; 
    for (i=0; i<=j; i++) {
      S12[Mc_AN][i+1][j+1] = LoS[m+i];
      S12[Mc_AN][j+1][i+1] = LoS[m+i];
    }
  }
