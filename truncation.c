  ...
  ****************************************************/
  ...
  if (2<=MD_iter && flag_GDC==0){
    ...
    *****************************/    
    ...
    Set_Allocate_Atom2CPU(MD_iter,0,0);
  }
    ...
    ****************************************************/
    ...
    do{
      ...
      **********************/
      ...
      else if (TFNAN==TFNAN2 && (Solver==2 || Solver==3 || Solver==4 || Solver==7) )              po++;
  ...
  ****************************************************/
  ...
  if (MD_iter==1 && UCell_flag==1){
    ...
    *************************************/
    ...
    if (myid==Host_ID)  printf("<UCell_Box> Info. of cutoff energy and num. of grids\n");
...
void Check_System()
{
  ...
  if (myid==Host_ID) printf("<Check_System> The system is %s\n",s_vec[num]);
}
...
#ifdef TRAN
    if (Solver==4) {
      TRAN_adjust_Grid_Origin(mpi_comm_level1, Grid_Origin);
    }
#endif
...
#pragma omp flush(Mc_AN0)
  ...
  ****************************************************/
  ...
  for (ct_AN=1; ct_AN<=atomnum; ct_AN++){
    ...
    if (myid==Host_ID && estimate_switch==0){
...
{ 
  ...
  /*********************************
   ...
   int po_ID[Matomnum+1][numprocs]
   int FS_Snd_Num[numprocs]
  *********************************/

   po_ID = (int**)malloc(sizeof(int*)*(Matomnum+1));
   for (Mc_AN=0; Mc_AN<=Matomnum; Mc_AN++){
     po_ID[Mc_AN] = (int*)malloc(sizeof(int)*numprocs);
   }

   FS_Snd_Num = (int*)malloc(sizeof(int)*numprocs);
  ...
  *************************************************/
  ...
  /*****************************************
    ...
    int Snd_MAN[numprocs][FS_Snd_Num[ID]+1]
    int Snd_GAN[numprocs][FS_Snd_Num[ID]+1]
  *****************************************/
  ...
  for (ID=0; ID<numprocs; ID++){
    Snd_MAN[ID] = (int*)malloc(sizeof(int)*(FS_Snd_Num[ID]+1));
  }

  Snd_GAN = (int**)malloc(sizeof(int*)*numprocs);
  for (ID=0; ID<numprocs; ID++){
    Snd_GAN[ID] = (int*)malloc(sizeof(int)*(FS_Snd_Num[ID]+1));
  }
  ...
  *************************************************/
  ...
  for (Mc_AN=1; Mc_AN<=Matomnum; Mc_AN++){
    ...
    for (Lh_AN=0; Lh_AN<=FNAN[Gc_AN]; Lh_AN++){
      ...
      ID1 = G2ID[Gh_AN];
      if (po_ID[Mc_AN][ID1]==0 && ID1!=myid){
        F_Snd_Num[ID1] = F_Snd_Num[ID1] + 1;
        /* Suffix "Num" starts from 0. */ 
        Num = F_Snd_Num[ID1] - 1; 
        Snd_MAN[ID1][Num] = Mc_AN;
        Snd_GAN[ID1][Num] = Gc_AN;
        po_ID[Mc_AN][ID1] = 1;
      }
    }
  }
  ...
  *************************************************/
  ...
  for (Mc_AN=1; Mc_AN<=Matomnum; Mc_AN++){
    ...
    for (Lh_AN=(FNAN[Gc_AN]+1); Lh_AN<=(FNAN[Gc_AN]+SNAN[Gc_AN]); Lh_AN++){
      ...
      ID1 = G2ID[Gh_AN];
      if (po_ID[Mc_AN][ID1]==0 && ID1!=myid){
        S_Snd_Num[ID1] = S_Snd_Num[ID1] + 1;
        /* Suffix "Num" starts from 0. */
        Num = F_Snd_Num[ID1] + S_Snd_Num[ID1] - 1;
        Snd_MAN[ID1][Num] = Mc_AN;
        Snd_GAN[ID1][Num] = Gc_AN;
        po_ID[Mc_AN][ID1] = 1;
      }
  ...
  *************************************************/
  ...
  for (ID=0; ID<numprocs; ID++){
    if (ID!=0){
      IDS = (myid + ID) % numprocs;
      IDR = (myid - ID + numprocs) % numprocs;
      MPI_Isend(&F_Snd_Num[IDS],1,MPI_INT,IDS,tag,mpi_comm_level1,&request);
      MPI_Recv( &F_Rcv_Num[IDR],1,MPI_INT,IDR,tag,mpi_comm_level1,&stat); 
  ...
  for (ID=0; ID<numprocs; ID++){
    if (ID!=0){
      IDS = (myid + ID) % numprocs;
      IDR = (myid - ID + numprocs) % numprocs;
      MPI_Isend(&S_Snd_Num[IDS],1,MPI_INT,IDS,tag,mpi_comm_level1,&request);
      MPI_Recv( &S_Rcv_Num[IDR],1,MPI_INT,IDR,tag,mpi_comm_level1,&stat); 
      MPI_Wait(&request,&stat);
    }
  }
...
void allocate_grids2atoms(int MD_iter)
{
  ...
  /******************************************************
    find the perpendicular unit vector to the bc-plane
  ******************************************************/
