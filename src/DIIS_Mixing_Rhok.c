...
static void DIIS_Mixing_Rhok1(int SCF_iter,
			      double Mix_wgt,
			      double *****ReRhok,
			      double *****ImRhok,
			      double ****ReBestRhok,
...
void DIIS_Mixing_Rhok1(int SCF_iter,
                      double Mix_wgt,
                      double *****ReRhok,
                      double *****ImRhok,
                      double ****ReBestRhok,
...
#pragma omp parallel shared(Nthrds0)
    ...
    /* allocation of array */
    My_A_thrds = (double*)malloc(sizeof(double)*Nthrds0);
      
    for (SCFi=1; SCFi<=NumMix; SCFi++){
      for (SCFj=SCFi; SCFj<=NumMix; SCFj++){

        for (Nloop=0; Nloop<Nthrds0; Nloop++) My_A_thrds[Nloop] = 0.0;

#pragma omp parallel shared(SCFi,SCFj,My_A_thrds,Kerker_weight,Residual_ReRhok,Residual_ImRhok,knum_full,My_NGrid2_Poisson,Ngrid1,Ngrid3) private(OMPID,Nthrds,Nprocs,k,spin,k2,k1,k3,weight,re1,im1,re2,im2,tmp0)
	{
	  /* get info. on OpenMP */ 
	  ...
	  for (k=OMPID*knum_full/Nthrds; k<(OMPID+1)*knum_full/Nthrds; k++){
        ...
        for (Nloop=0; Nloop<Nthrds0; Nloop++){
          My_A[SCFi][SCFj] += My_A_thrds[Nloop];
	}
