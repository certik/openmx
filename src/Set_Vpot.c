...
void Set_Vpot(int SCF_iter, int XC_P_switch, double *****CDM)
{
  ...
  ****************************************************/
  ...
  int Nc,Nd,n1,n2,n3,Cwan,spin,MN;
  int h_AN,Gh_AN,Hwan,Rnh,size1,size2;
  int My_Max,Max_Size;
  int hNgrid1,hNgrid2,hNgrid3;
  double Gx,Gy,Gz,sum,x,y,z,dx,dy,dz,r;
  double Cxyz[4];
  double *tmp_array;
  double *tmp_array2;
  double **AtomVNA_Grid;
...
#pragma omp parallel shared(AtomVNA_Grid,GridN_Atom,atv,Gxyz,Gc_AN,Cwan,Mc_AN) private(OMPID,Nthrds,Nprocs,Nc,GNc,GRc,Cxyz,dx,dy,dz,r)
      {
...
#pragma omp parallel shared(E_Field,My_Cell1,Num_Cells0,Ngrid2,Ngrid3,VEF_Grid,length_gtv,hNgrid1,hNgrid2,hNgrid3) private(OMPID,Nthrds,Nprocs,nmax,n,i,j,k,ri,MN)
    {
