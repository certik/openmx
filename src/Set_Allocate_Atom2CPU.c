...
void GDC_Allocation(int MD_iter, int isw, int NL_switch)
{
  ...
  *****************************************/
  ...
  if (myid==Host_ID && 1<=MD_iter) printf("\n\n\n");
...
#if  0
  printf("%d: Set_Allocate_Atom2CPU start\n",myid);
  ...
  ****************************************************/
  ...
  if (isw == 0){
    ...
    *****************************************/
    ...
    else if ( NL_switch==1 && (Solver==2 || Solver==3 || Solver==7) ){
    ...
    /* the number of atoms weighted by (# of orbitals)^2 */
      ...
      for (ct_AN=1; ct_AN<=atomnum; ct_AN++){
        num1 = FNAN[ct_AN];
        num1 *= num1;
        weight[ct_AN] = 0.0001*(double)num1;
        sum += weight[ct_AN];
      }
      av_num_atoms = sum/(double)numprocs;
    ...
    /*
    for (ct_AN=1; ct_AN<=atomnum; ct_AN++){
      printf("myid=%2d ct_AN=%2d weight=%15.12f\n",myid,ct_AN,weight[ct_AN]);
    }
    */
    ...
    for (i=1; i<=numprocs*num; i++){
      ...
      /*
      if (myid==0){
        printf("ia=%i  i=%i  bound=%15.12f  num_atom=%10.6f\n",ia,i,bound[i],num_atom[i]);
      }
      */
  ...
  while (bin[numprocs]==0) {
    ...
    for (k=1; k<=numprocs; k++){
...
#if 0
      printf("%d: k=%d po=%d %d Matomnum2=%d\n",myid, k,po0,po1,Matomnum2);
#endif
      ...
      for (ct_AN=1; ct_AN<=atomnum; ct_AN++){
        if (   bound[po0]<=Cell_Gxyz[ct_AN][1] 
          ...
          Matomnum2++; 
          G2ID[ct_AN] = k-1;
          if (myid==(k-1)) M2G[Matomnum2] = ct_AN;
        }
      }
#else
...
#if 0
    printf("%d: MatomN, WMatomN=%d %f\n",myid, MatomN[myid] , WMatomN[myid] );
#endif
    ...
    *****************************************/
...
#if  0
    for (ID=0; ID<numprocs; ID++){
     printf(" proccesor = %3d    # of atoms = %4d   %16.5f\n",ID,MatomN[ID],WMatomN[ID]);
    }
#endif
    ...
    if (myid==Host_ID){
      for (ct_AN=1; ct_AN<=atomnum; ct_AN++){
        printf("myid=%3d  time_per_atom=%12.5f\n",ct_AN,time_per_atom[ct_AN]);
      }
    }
    */
...
#pragma omp parallel shared(level_stdout,NOHS_C,cell_atv,Gxyz,TN_Rn,atomnum,SNAN,FNAN,Spe_Total_CNO,Spe_Atom_Cut1,WhatSpecies,M2G,BCR2,Matomnum) private(i,ct_AN,wanA,rcutA,tno1,j,wanB,rcutB,rcut,rcut2,tno2,Rn,dx,dy,dz,r2)
  {
	...
	for (Rn=0; Rn<TN_Rn; Rn++){
	  ...
	  if ((ct_AN==j) && Rn==0){
	    ...
	    if (0.01<r2 && r2<=rcut2)    FNAN[ct_AN] += tno1*tno2;
	    if (NOHS_C==1){
